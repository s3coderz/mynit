...
public class CountryUtil {
	public static List<HashMap<String, String>> prepareList(Context context) {
		List<HashMap<String, String>> countryList = new ArrayList<HashMap<String, String>>();
		HashMap<String, String> countryNames = getCountryList();
		// getNetworkCountryIso
		String[] rl = context.getResources().getStringArray(
				R.array.CountryCodes);
		for (int i = 0; i < rl.length; i++) {
			String[] g = rl[i].split(",");
			String code = g[0].trim();
			String country = countryNames.get(g[1].trim().toUpperCase());
		...
		Log.i("", "Country list count : " + countryList.size());
		return countryList;
	}

	private static HashMap<String, String> getCountryList() {
		HashMap<String, String> map = new HashMap<String, String>();
	...
	public static String getUserCountryName(Context context) {
		String country = getUserCountryISO(context);
		if (country != null && !country.equalsIgnoreCase("")
				&& country.length() == 2) {
			HashMap<String, String> map = getCountryList();
			String code = "";
	...
	private static String getUserCountryISO(Context context) {
		try {
	...
	public static String getUserCountryCode(Context context) {
		String iso = getUserCountryISO(context);
		String[] rl = context.getResources().getStringArray(
				R.array.CountryCodes);
		for (int i = 0; i < rl.length; i++) {
			String[] g = rl[i].split(",");
			if(g[1].equalsIgnoreCase(iso)){
				return g[0].trim();
			}
		}
		return "";
